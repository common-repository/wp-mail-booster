...
class Google_Model implements ArrayAccess {
	...
	/**
	 * This variable for internal gapi mappings
	 *
	 * @var array
	 */
	protected $internal_gapi_mappings = array();
	/**
	 * This variable for model data
	 *
	 * @var array
	 */
	protected $modelData = array(); // @codingStandardsIgnoreLine
	/**
	 * This variable for processd
	 *
	 * @var array
	 */
	...
	public function __get( $key ) {
		...
		if ( isset( $this->$keyTypeName ) && ! isset( $this->processed[ $key ] ) ) {// @codingStandardsIgnoreLine
			...
			} elseif ( isset( $this->$keyDataType ) &&// @codingStandardsIgnoreLine
			( $this->$keyDataType == 'array' || $this->$keyDataType == 'map' ) ) {// @codingStandardsIgnoreLine
				$val = array();
			} else {
				$val = null;
			}
			...
			if ( $this->isAssociativeArray( $val ) ) {
				if ( isset( $this->$keyDataType ) && 'map' == $this->$keyDataType ) { // @codingStandardsIgnoreLine
					foreach ( $val as $arrayKey => $arrayItem ) { // @codingStandardsIgnoreLine
						$this->modelData[ $key ][ $arrayKey ] = // @codingStandardsIgnoreLine
						$this->createObjectFromName( $keyTypeName, $arrayItem );// @codingStandardsIgnoreLine
					}
				} else {
				...
				foreach ( $val as $arrayIndex => $arrayItem ) {// @codingStandardsIgnoreLine
					$arrayObject[ $arrayIndex ] =// @codingStandardsIgnoreLine
					$this->createObjectFromName( $keyTypeName, $arrayItem );// @codingStandardsIgnoreLine
				}
	...
	protected function mapTypes( $array ) {
		...
		foreach ( $array as $key => $val ) {
			if ( property_exists( $this, $this->keyType( $key ) ) ) {
				$propertyClass = $this->{$this->keyType( $key )}; // @codingStandardsIgnoreLine
				$dataType      = $this->{$this->dataType( $key )}; // @codingStandardsIgnoreLine
				if ( $dataType == 'array' || $dataType == 'map' ) { // @codingStandardsIgnoreLine
					$this->$key = array();
					foreach ( $val as $itemKey => $itemVal ) { // @codingStandardsIgnoreLine
						if ( $itemVal instanceof $propertyClass ) { // @codingStandardsIgnoreLine
							$this->{$key}[ $itemKey ] = $itemVal; // @codingStandardsIgnoreLine
						} else {
							$this->{$key}[ $itemKey ] = new $propertyClass( $itemVal ); // @codingStandardsIgnoreLine
						}
					}
				} elseif ( $val instanceof $propertyClass ) { // @codingStandardsIgnoreLine
					$this->$key = $val;
				} else {
					$this->$key = new $propertyClass( $val ); // @codingStandardsIgnoreLine
				}
				unset( $array[ $key ] );
			} elseif ( property_exists( $this, $key ) ) {
				$this->$key = $val;
				unset( $array[ $key ] );
			} elseif ( property_exists( $this, $camelKey = $this->camelCase( $key ) ) ) { // @codingStandardsIgnoreLine
	...
	/**
	 * Function to check datatype of key.
	 *
	 * @param string $key .
	 */
