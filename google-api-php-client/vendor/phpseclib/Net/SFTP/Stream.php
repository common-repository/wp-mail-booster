...
/**
 * This file for SFTP Stream Wrapper.
 *
 * @author  Tech Banker
...
class Stream {
	 ...
	 * @var array
	 */
	static $instances; // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $sftp; // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $path; // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $pos; // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $size; // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $entries; // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $eof; // @codingStandardsIgnoreLine
	 ...
	 * @access public
	 */
	var $context; // @codingStandardsIgnoreLine
	 ...
	 * @access public
	 */
	var $notification; // @codingStandardsIgnoreLine
	 ...
	 * @access public
	 */
	static function register( $protocol = 'sftp' ) { // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	private function _parse_path( $path ) { // @codingStandardsIgnoreLine
		$orig = $path;
		extract( parse_url( $path ) + array( 'port' => 22 ) ); // @codingStandardsIgnoreLine
		if ( isset( $query ) ) {
			$path .= '?' . $query;
		...
		if ( isset( $fragment ) ) {
			$path .= '#' . $fragment;
		} elseif ( $orig[ strlen( $orig ) - 1 ] == '#' ) { // @codingStandardsIgnoreLine
			$path .= '#';
		}
		...
		if ( '$' == $host[0] ) { // WPCS:Loose comparison ok .
			$host = substr( $host, 1 );
			global $$host;
	 ...
	 * @access public
	 */
	public function _stream_open( $path, $mode, $options, &$opened_path ) { // @codingStandardsIgnoreLine
		...
		if ( false === $this->size ) {
			if ( 'r' == $this->mode[0] ) { // WPCS:Loose comparison ok .
				return false;
			} else {
		...
		$this->pos = 'a' != $this->mode[0] ? 0 : $this->size; // WPCS:Loose comparison ok .
	 ...
	 * @access public
	 */
	public function _stream_read( $count ) { // @codingStandardsIgnoreLine
		switch ( $this->mode ) {
			case 'w':
			case 'a':
			case 'x':
	 ...
	 * @access public
	 */
	public function _stream_write( $data ) { // @codingStandardsIgnoreLine
		switch ( $this->mode ) {
			case 'r':
				return false;
		}
	 ...
	 * @access public
	 */
	public function _stream_tell() { // @codingStandardsIgnoreLine
		return $this->pos;
	}
	 ...
	 * @access public
	 */
	public function _stream_eof() { // @codingStandardsIgnoreLine
		return $this->eof;
	}
	 ...
	 * @access public
	 */
	public function _stream_seek( $offset, $whence ) { // @codingStandardsIgnoreLine
		switch ( $whence ) {
			case SEEK_SET:
	 ...
	 * @access public
	 */
	public function _stream_metadata( $path, $option, $var ) { // @codingStandardsIgnoreLine
		$path = $this->_parse_path( $path );
		if ( false === $path ) {
			return false;
		}
	 ...
	 * @access public
	 */
	public function _stream_cast( $cast_as ) { // @codingStandardsIgnoreLine
	 ...
	 * @access public
	 */
	public function _stream_lock( $operation ) { // @codingStandardsIgnoreLine
		return false;
	}
	 ...
	 * @access public
	 */
	public function _rename( $path_from, $path_to ) { // @codingStandardsIgnoreLine
		$path1 = parse_url( $path_from ); // @codingStandardsIgnoreLine
		$path2 = parse_url( $path_to ); // @codingStandardsIgnoreLine
		unset( $path1['path'], $path2['path'] );
		if ( $path1 != $path2 ) { // WPCS:Loose comparison ok .
			return false;
		}

		$path_from = $this->_parse_path( $path_from );
		$path_to   = parse_url( $path_to ); // @codingStandardsIgnoreLine
		if ( false === $path_from ) {
			return false;
		}
	 ...
	 * @access public
	 */
	public function _dir_opendir( $path, $options ) { // @codingStandardsIgnoreLine
		$path = $this->_parse_path( $path );
		if ( false === $path ) {
			return false;
		}
	 ...
	 * @access public
	 */
	public function _dir_readdir() { // @codingStandardsIgnoreLine
	 ...
	 * @access public
	 */
	public function _dir_rewinddir() { // @codingStandardsIgnoreLine
		$this->pos = 0;
		return true;
	}
	 ...
	 * @access public
	 */
	public function _dir_closedir() { // @codingStandardsIgnoreLine
		return true;
	}
	 ...
	 * @access public
	 */
	public function _mkdir( $path, $mode, $options ) { // @codingStandardsIgnoreLine
		$path = $this->_parse_path( $path );
		if ( false === $path ) {
			return false;
		}
	 ...
	 * @access public
	 */
	public function _rmdir( $path, $options ) { // @codingStandardsIgnoreLine
		$path = $this->_parse_path( $path );
		if ( false === $path ) {
			return false;
		}
	 ...
	 * @access public
	 */
	public function _stream_flush() { // @codingStandardsIgnoreLine
		return true;
	}
	 ...
	 * @access public
	 */
	public function _stream_stat() { // @codingStandardsIgnoreLine
	 ...
	 * @access public
	 */
	public function _unlink( $path ) { // @codingStandardsIgnoreLine
		$path = $this->_parse_path( $path );
		if ( false === $path ) {
			return false;
		}
	 ...
	 * @access public
	 */
	public function _url_stat( $path, $flags ) { // @codingStandardsIgnoreLine
		$path = $this->_parse_path( $path );
		if ( false === $path ) {
			return false;
		}
	 ...
	 * @access public
	 */
	public function _stream_truncate( $new_size ) { // @codingStandardsIgnoreLine
	 ...
	 * @access public
	 */
	public function _stream_set_option( $option, $arg1, $arg2 ) { // @codingStandardsIgnoreLine
		return false;
	}
	 ...
	 * @access public
	 */
	public function _stream_close() { // @codingStandardsIgnoreLine
	}
	...
	public function __call( $name, $arguments ) {
		if ( defined( 'NET_SFTP_STREAM_LOGGING' ) ) {
			echo $name . '('; // @codingStandardsIgnoreLine
			$last = count( $arguments ) - 1;
			foreach ( $arguments as $i => $argument ) {
				var_export( $argument ); // @codingStandardsIgnoreLine
				if ( $i != $last ) { // WPCS:Loose comparison ok .
					echo ',';
				}
			}
			echo ")\r\n";
