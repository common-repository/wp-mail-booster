...
if ( ! class_exists( 'Token_Manager_Mail_Booster' ) ) {
	/**
	 ...
	 * @author Tech Banker
	 */
	class Token_Manager_Mail_Booster {
		/**
		 * Manage client id.
		 *
		 * @access   public
		...
		public $callback_uri;
			 ...
			 *
			 * @param Mail_Booster_Manage_Token $client_id .
			 * @param Mail_Booster_Manage_Token $client_secret .
			 * @param Mail_Booster_Manage_Token $authorization_token .
			 * @param Mail_Booster_Manage_Token $callback_uri .
			 */
		...
		public function check_access_token() {
			$expiry_time   = intval( $this->authorization_token->retrieve_token_expiry_time_mail_booster() ) - 60;
		...
		public function receive_decode_authorization_token( $new_token ) {
			...
			$this->get_authorization_token()->set_token_expirytime_mail_booster( $changed_expiry_time );
			...
			$this->get_authorization_token()->set_access_token_mail_booster( $new_access_token );

			// Update refresh token.
			if ( isset( $new_token->{'refresh_token'} ) ) {
				$new_refresh_token = $new_token->{'refresh_token'};
				$this->get_authorization_token()->set_refresh_token_mail_booster( $new_refresh_token );
			}
		}
		/**
		...
		public function get_refresh_token() {
			...
			$response       = Mail_Booster_Zend_Mail_Helper::retrieve_body_from_response_mail_booster( $refresh_uri, $configurations );
