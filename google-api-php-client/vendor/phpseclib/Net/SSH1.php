...
/**
 * This file for Pure-PHP implementation of SSHv1.
 *
 * @author  Tech Banker
...
class SSH1 {
	...
	*/
	 ...
	 * @access private
	 */
	var $identifier = 'SSH-1.5-phpseclib'; // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $crypto = false; // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $server_key_public_exponent; // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $server_key_public_modulus; // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $host_key_public_exponent; // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $host_key_public_modulus; // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $supported_ciphers = array( // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $supported_authentications = array( // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $server_identification = ''; // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $protocol_flags = array(); // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $protocol_flag_log = array(); // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $connectionTimeout; // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	var $cipher; // @codingStandardsIgnoreLine
	...
	public function __construct( $host, $port = 22, $timeout = 10, $cipher = self::CIPHER_3DES ) {
		...
		$this->host              = $host;
		$this->port              = $port;
		$this->connectionTimeout = $timeout; // @codingStandardsIgnoreLine
		$this->cipher            = $cipher;
	}
	...
	private function _connect() { // @codingStandardsIgnoreLine
		$this->fsock = @fsockopen( $this->host, $this->port, $errno, $errstr, $this->connectionTimeout ); // @codingStandardsIgnoreLine
		if ( ! $this->fsock ) {
			user_error( rtrim( "Cannot connect to {$this->host}:{$this->port}. Error $errno. $errstr" ) ); // @codingStandardsIgnoreLine
			return false;
		}

		$this->server_identification = $init_line = fgets( $this->fsock, 255 ); // @codingStandardsIgnoreLine
		...
		if ( 1 != $parts[1][0] ) { // WPCS:Loose comparison ok .
		...
		if ( NET_SSH1_SMSG_PUBLIC_KEY != $response[ self::RESPONSE_TYPE ] ) { // WPCS:Loose comparison ok .
		...
		extract( unpack( 'Nsupported_ciphers_mask', $this->_string_shift( $response[ self::RESPONSE_DATA ], 4 ) ) ); // @codingStandardsIgnoreLine

		foreach ( $this->supported_ciphers as $mask => $name ) {
			if ( ( $supported_ciphers_mask & ( 1 << $mask ) ) == 0 ) { // WPCS:Loose comparison ok .
		...
		extract( unpack( 'Nsupported_authentications_mask', $this->_string_shift( $response[ self::RESPONSE_DATA ], 4 ) ) ); // @codingStandardsIgnoreLine
		foreach ( $this->supported_authentications as $mask => $name ) {
			if ( ( $supported_authentications_mask & ( 1 << $mask ) ) == 0 ) { // WPCS:Loose comparison ok .
		...
		if ( NET_SSH1_SMSG_SUCCESS != $response[ self::RESPONSE_TYPE ] ) { // WPCS:Loose comparison ok .
	...
	public function login( $username, $password = '' ) {
		...
		if ( NET_SSH1_SMSG_SUCCESS == $response[ self::RESPONSE_TYPE ] ) { // WPCS:Loose comparison ok .
			...
			return true;
		} elseif ( NET_SSH1_SMSG_FAILURE != $response[ self::RESPONSE_TYPE ] ) { // WPCS:Loose comparison ok .
		...
		if ( defined( 'NET_SSH1_LOGGING' ) && NET_SSH1_LOGGING == self::LOG_COMPLEX ) { // WPCS:Loose comparison ok .
			$data = pack( 'CNa*', NET_SSH1_CMSG_AUTH_PASSWORD, strlen( 'password' ), 'password' );
	...
	public function exec( $cmd, $block = true ) {
		...
		if ( false !== $response ) {
			do {
				...
				$response = $this->_get_binary_packet();
			} while ( is_array( $response ) && NET_SSH1_SMSG_EXITSTATUS != $response[ self::RESPONSE_TYPE ] ); // WPCS:Loose comparison ok .
		}
	...
	public function read( $expect, $mode = self::READ__SIMPLE ) {
		...
		$match = $expect;
		while ( true ) {
			if ( $mode == self::READ__REGEX ) { // @codingStandardsIgnoreLine
				preg_match( $expect, $this->interactiveBuffer, $matches ); // @codingStandardsIgnoreLine
			...
			if ( true === $response ) {
				return $this->_string_shift( $this->interactiveBuffer, strlen( $this->interactiveBuffer ) ); // @codingStandardsIgnoreLine
			}
			$this->interactiveBuffer .= substr( $response[ self::RESPONSE_DATA ], 4 ); // @codingStandardsIgnoreLine
		}
	}
	 ...
	 * @access public
	 */
	public function interactiveWrite( $cmd ) { // @codingStandardsIgnoreLine
		if ( ! ( $this->bitmap & self::MASK_LOGIN ) ) {
			user_error( 'Operation disallowed prior to login()' );
			return false;
		}
	 ...
	 * @access public
	 */
	public function interactiveRead() { // @codingStandardsIgnoreLine
		if ( ! ( $this->bitmap & self::MASK_LOGIN ) ) {
			user_error( 'Operation disallowed prior to login()' );
			return false;
		}
	 ...
	 * @access private
	 */
	private function _disconnect( $msg = 'Client Quit' ) { // @codingStandardsIgnoreLine
		if ( $this->bitmap ) {
	...
	private function _get_binary_packet() { // @codingStandardsIgnoreLine
		...
		$start = strtok( microtime(), ' ' ) + strtok( '' ); // http://php.net/microtime#61838 .
		$data  = fread( $this->fsock, 4 ); // @codingStandardsIgnoreLine
		if ( strlen( $data ) < 4 ) {
			return false;
		}
		...
		while ( $length > 0 ) {
			$temp    = fread( $this->fsock, $length ); // @codingStandardsIgnoreLine
			$raw    .= $temp;
			$length -= strlen( $temp );
		}
	 ...
	 * @access private
	 */
	private function _send_binary_packet( $data ) { // @codingStandardsIgnoreLine
		if ( feof( $this->fsock ) ) {
			return false;
		}
	 ...
	 * @access private
	 */
	private function _crc( $data ) { // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	private function _rsa_crypt( $m, $key ) { // @codingStandardsIgnoreLine
		$modulus = $key[1]->toBytes();
		$length  = strlen( $modulus ) - strlen( $m ) - 3;
		$random  = '';
		while ( strlen( $random ) != $length ) { // @codingStandardsIgnoreLine
	 ...
	 * @access public
	 */
	public function getServerKeyPublicExponent( $raw_output = false ) { // @codingStandardsIgnoreLine
	 ...
	 * @access public
	 */
	public function getServerKeyPublicModulus( $raw_output = false ) { // @codingStandardsIgnoreLine
	 ...
	 * @access public
	 */
	public function getHostKeyPublicExponent( $raw_output = false ) { // @codingStandardsIgnoreLine
	 ...
	 * @access public
	 */
	public function getHostKeyPublicModulus( $raw_output = false ) { // @codingStandardsIgnoreLine
	 ...
	 * @access public
	 */
	public function getSupportedCiphers( $raw_output = false ) { // @codingStandardsIgnoreLine
	 ...
	 * @access public
	 */
	public function getSupportedAuthentications( $raw_output = false ) { // @codingStandardsIgnoreLine
	 ...
	 * @access private
	 */
	private function _append_log( $protocol_flags, $message ) { // @codingStandardsIgnoreLine
		switch ( NET_SSH1_LOGGING ) {
