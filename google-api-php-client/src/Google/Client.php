...
class Google_Client {
	...
	/**
	 * Variable for Scopes requested by the client .
	 *
	 * @var array $scopes .
	 */
	protected $requestedScopes = []; // @codingStandardsIgnoreLine
	 ...
	 */
	public function fetchAccessTokenWithAuthCode( $code ) { // @codingStandardsIgnoreLine
		...
		$httpHandler = HttpHandlerFactory::build( $this->getHttpClient() ); // @codingStandardsIgnoreLine
		$creds       = $auth->fetchAuthToken( $httpHandler ); // @codingStandardsIgnoreLine
	 ...
	 * @deprecated
	 */
	public function refreshTokenWithAssertion() { // @codingStandardsIgnoreLine
	 ...
	 *
	 * @param string $authHttp Credentials optional .
	 * @return array access token .
	 * @throws DomainException .
	 */
	public function fetchAccessTokenWithAssertion( ClientInterface $authHttp = null ) { // @codingStandardsIgnoreLine
		...
		$httpHandler = HttpHandlerFactory::build( $authHttp ); // @codingStandardsIgnoreLine
		$creds       = $credentials->fetchAuthToken( $httpHandler ); // @codingStandardsIgnoreLine
		if ( $creds && isset( $creds['access_token'] ) ) {
			$creds['created'] = time();
			$this->setAccessToken( $creds );
		}
	...
	public function refreshToken( $refreshToken ) { // @codingStandardsIgnoreLine
		return $this->fetchAccessTokenWithRefreshToken( $refreshToken ); // @codingStandardsIgnoreLine
	}
	 ...
	 */
	public function fetchAccessTokenWithRefreshToken( $refreshToken = null ) { // @codingStandardsIgnoreLine
		if ( null === $refreshToken ) { // @codingStandardsIgnoreLine
			if ( ! isset( $this->token['refresh_token'] ) ) {
				throw new LogicException(
					'refresh token must be passed in or set as part of setAccessToken'
				);
			}
			$refreshToken = $this->token['refresh_token']; // @codingStandardsIgnoreLine
		}
		$this->getLogger()->info( 'OAuth2 access token refresh' );
		$auth = $this->getOAuth2Service();
		$auth->setRefreshToken( $refreshToken ); // @codingStandardsIgnoreLine
		...
		if ( $creds && isset( $creds['access_token'] ) ) {
			...
			if ( ! isset( $creds['refresh_token'] ) ) {
				$creds['refresh_token'] = $refreshToken; // @codingStandardsIgnoreLine
			}
			$this->setAccessToken( $creds );
		}
	 ...
	 * @return string
	 */
	public function createAuthUrl( $scope = null ) { // @codingStandardsIgnoreLine
		if ( empty( $scope ) ) {
		...
		$approvalPrompt = $this->config['prompt'] // @codingStandardsIgnoreLine
		? null
		: $this->config['approval_prompt'];

		// include_granted_scopes should be string "true", string "false", or null .
		$includeGrantedScopes = $this->config['include_granted_scopes'] === null // @codingStandardsIgnoreLine
		? null
		: var_export( $this->config['include_granted_scopes'], true ); //@codingStandardsIgnoreLine

		$params = array_filter(
			[
				'access_type'            => $this->config['access_type'],
				'approval_prompt'        => $approvalPrompt, // @codingStandardsIgnoreLine
				'hd'                     => $this->config['hd'],
				'include_granted_scopes' => $includeGrantedScopes, // @codingStandardsIgnoreLine
	...
	public function authorize( ClientInterface $http = null ) {
		...
		if ( $this->isUsingApplicationDefaultCredentials() ) {
			$credentials = $this->createApplicationDefaultCredentials();
		} elseif ( $token = $this->getAccessToken() ) { // @codingStandardsIgnoreLine
		...
		$authHandler = $this->getAuthHandler(); // @codingStandardsIgnoreLine

		if ( $credentials ) {
			$callback = $this->config['token_callback'];
			$http     = $authHandler->attachCredentials( $http, $credentials, $callback ); // @codingStandardsIgnoreLine
		} elseif ( $token ) {
			$http = $authHandler->attachToken( $http, $token, (array) $scopes ); // @codingStandardsIgnoreLine
		} elseif ( $key = $this->config['developer_key'] ) { // @codingStandardsIgnoreLine
			$http = $authHandler->attachKey( $http, $key ); // @codingStandardsIgnoreLine
		}
	 ...
	 */
	public function useApplicationDefaultCredentials( $useAppCreds = true ) { // @codingStandardsIgnoreLine
		$this->config['use_application_default_credentials'] = $useAppCreds; // @codingStandardsIgnoreLine
	}
	 ...
	 * @see https://developers.google.com/identity/protocols/application-default-credentials
	 */
	public function isUsingApplicationDefaultCredentials() { // @codingStandardsIgnoreLine
		return $this->config['use_application_default_credentials'];
	}

	/**
	 * This function is to set access token .
	 *
	 * @param string|array $token .
	 * @throws InvalidArgumentException $token .
	 */
	public function setAccessToken( $token ) { // @codingStandardsIgnoreLine
		if ( is_string( $token ) ) {
			if ( $json = json_decode( $token, true ) ) { // @codingStandardsIgnoreLine
				$token = $json;
			} else {
	...
	/**
	 * This function is to get access token .
	 */
	public function getAccessToken() { // @codingStandardsIgnoreLine
		return $this->token;
	}
	/**
	 * This function is to get refresh token .
	 */
	...
	public function isAccessTokenExpired() { // @codingStandardsIgnoreLine
		...
		$created = 0;
		if ( isset( $this->token['auth_token_expires'] ) ) {
			$created = $this->token['auth_token_expires'];
			...
			$idToken = $this->token['id_token']; // @codingStandardsIgnoreLine
			if ( substr_count( $idToken, '.' ) == 2 ) { // @codingStandardsIgnoreLine
				$parts   = explode( '.', $idToken ); // @codingStandardsIgnoreLine
	...
	public function setAuth( $auth ) { // @codingStandardsIgnoreLine
		throw new BadMethodCallException(
			'This function no longer exists. See UPGRADING.md for more information'
		);
	}
	...
	public function setClientId( $clientId ) { // @codingStandardsIgnoreLine
		$this->config['client_id'] = $clientId; // @codingStandardsIgnoreLine
	}
	/**
	 * This function is to get client id .
	 */
	public function getClientId() { // @codingStandardsIgnoreLine
	...
	public function setClientSecret( $clientSecret ) { // @codingStandardsIgnoreLine
		$this->config['client_secret'] = $clientSecret; // @codingStandardsIgnoreLine
	}
	/**
	 * This function is to get client secret .
	 */
	public function getClientSecret() { // @codingStandardsIgnoreLine
	...
	public function setRedirectUri( $redirectUri ) { // @codingStandardsIgnoreLine
		$this->config['redirect_uri'] = $redirectUri; // @codingStandardsIgnoreLine
	}
	/**
	 * This function is to get redirect uri .
	 */
	public function getRedirectUri() { // @codingStandardsIgnoreLine
	...
	/**
	 * This function is for set access type .
	 *
	...
	public function setAccessType( $accessType ) { // @codingStandardsIgnoreLine
		$this->config['access_type'] = $accessType; // @codingStandardsIgnoreLine
	}

	/**
	 * Function to set approval prompt .
	 *
	...
	public function setApprovalPrompt( $approvalPrompt ) { // @codingStandardsIgnoreLine
		$this->config['approval_prompt'] = $approvalPrompt; // @codingStandardsIgnoreLine
	}
	...
	public function setLoginHint( $loginHint ) { // @codingStandardsIgnoreLine
		$this->config['login_hint'] = $loginHint; // @codingStandardsIgnoreLine
	}
	...
	public function setApplicationName( $applicationName ) { // @codingStandardsIgnoreLine
		$this->config['application_name'] = $applicationName; // @codingStandardsIgnoreLine
	}
	...
	public function setRequestVisibleActions( $requestVisibleActions ) { // @codingStandardsIgnoreLine
		if ( is_array( $requestVisibleActions ) ) { // @codingStandardsIgnoreLine
			$requestVisibleActions = implode( ' ', $requestVisibleActions ); // @codingStandardsIgnoreLine
		}
		$this->config['request_visible_actions'] = $requestVisibleActions; // @codingStandardsIgnoreLine
	}
	...
	public function setDeveloperKey( $developerKey ) { // @codingStandardsIgnoreLine
		$this->config['developer_key'] = $developerKey; // @codingStandardsIgnoreLine
	}
	 ...
	 */
	public function setTokenCallback( callable $tokenCallback ) { // @codingStandardsIgnoreLine
		$this->config['token_callback'] = $tokenCallback; // @codingStandardsIgnoreLine
	}
	...
	public function revokeToken( $token = null ) { // @codingStandardsIgnoreLine
		$tokenRevoker = new Google_AccessToken_Revoke( // @codingStandardsIgnoreLine
			$this->getHttpClient()
		);

		return $tokenRevoker->revokeToken( $token ?: $this->getAccessToken() ); // @codingStandardsIgnoreLine
	}
	 ...
	 * isn't provided.
	 *
	 * @throws LogicException $idToken .
	 * @param string|null $idToken The token (id_token) that should be verified.
	 * @return array|false Returns the token payload as an array if the verification was
	 * successful, false otherwise.
	 */
	public function verifyIdToken( $idToken = null ) { // @codingStandardsIgnoreLine
		$tokenVerifier = new Google_AccessToken_Verify( // @codingStandardsIgnoreLine
			$this->getHttpClient(),
			$this->getCache(),
			$this->config['jwt']
		);

		if ( null === $idToken ) { // @codingStandardsIgnoreLine
			...
			$idToken = $token['id_token']; // @codingStandardsIgnoreLine
		}

		return $tokenVerifier->verifyIdToken( // @codingStandardsIgnoreLine
	...
	public function prepareScopes() { // @codingStandardsIgnoreLine
		...
		return implode( ' ', $this->requestedScopes ); // @codingStandardsIgnoreLine
	}

	/**
	 * Helper method to execute deferred HTTP requests.
	 *
	 * @param string $request Psr\Http\Message\RequestInterface|Google_Http_Batch .
	 * @throws Google_Exception $request .
	 * @param string $expectedClass .
	 * @return object of the type of the expected class or Psr\Http\Message\ResponseInterface.
	 */
	public function execute( RequestInterface $request, $expectedClass = null ) { // @codingStandardsIgnoreLine
		...
		return Google_Http_REST::execute( $http, $request, $expectedClass, $this->config['retry'] ); // @codingStandardsIgnoreLine
	}
	...
	public function isAppEngine() { // @codingStandardsIgnoreLine
		...
		strpos( $_SERVER['SERVER_SOFTWARE'], 'Google App Engine' ) !== false ); // WPCS: input var ok ,sanitization ok.
	}
	/**
	 * This function is to set config .
	 *
	 * @param array $name .
	 * @param array $value .
	 */
	public function setConfig( $name, $value ) { // @codingStandardsIgnoreLine
		$this->config[ $name ] = $value;
	}
	/**
	 * This function is to get config .
	 *
	 * @param array $name .
	 * @param array $default .
	 */
	public function getConfig( $name, $default = null ) { // @codingStandardsIgnoreLine
	 ...
	 * @throws LogicException $config .
	 */
	public function setAuthConfig( $config ) { // @codingStandardsIgnoreLine
		if ( is_string( $config ) ) {
			if ( ! file_exists( $config ) ) {
				throw new InvalidArgumentException( 'file does not exist' );
			}

			$json = file_get_contents( $config ); // @codingStandardsIgnoreLine

			if ( ! $config = json_decode( $json, true ) ) { // @codingStandardsIgnoreLine
				throw new LogicException( 'invalid json for auth config' );
			}
		}

		$key = isset( $config['installed'] ) ? 'installed' : 'web';
		if ( isset( $config['type'] ) && 'service_account' == $config['type'] ) { // WPCS:Loose comparison ok .
	 ...
	 */
	public function setSubject( $subject ) { // @codingStandardsIgnoreLine
	...
	/**
	 * This function is used to get oauth Service
	 *
	 * @return Google\Auth\OAuth2 implementation .
	 */
	public function getOAuth2Service() { // @codingStandardsIgnoreLine
		if ( ! isset( $this->auth ) ) {
	 ...
	 */
	protected function createOAuth2Service() { // @codingStandardsIgnoreLine
		$auth = new OAuth2(
			[
	 ...
	 */
	public function setCache( CacheItemPoolInterface $cache ) { // @codingStandardsIgnoreLine
		$this->cache = $cache;
	}
	...
	/**
	 * This function is used to set cache config .
	 *
	 * @param array $cacheConfig .
	 */
	public function setCacheConfig( array $cacheConfig ) { // @codingStandardsIgnoreLine
		$this->config['cache_config'] = $cacheConfig; // @codingStandardsIgnoreLine
	}
	...
	/**
	 * This function is to create default logger .
	 */
	protected function createDefaultLogger() { // @codingStandardsIgnoreLine
	...
	/**
	 * This function is to create default cache .
	 */
	protected function createDefaultCache() { // @codingStandardsIgnoreLine
	 ...
	 */
	public function setHttpClient( ClientInterface $http ) { // @codingStandardsIgnoreLine
		$this->http = $http;
	}

	/**
	 * This function to get http client
	 *
	 * @return GuzzleHttp\ClientInterface implementation .
	 */
	public function getHttpClient() { // @codingStandardsIgnoreLine
		if ( null === $this->http ) {
	...
	/**
	 * This function is to create default http client .
	 */
	protected function createDefaultHttpClient() { // @codingStandardsIgnoreLine
	...
	/**
	 * This function is to create default apllication credentials .
	 *
	 * @throws DomainException .
	 */
	private function createApplicationDefaultCredentials() { // @codingStandardsIgnoreLine
		$scopes     = $this->prepareScopes();
		$sub        = $this->config['subject'];
		$signingKey = $this->config['signing_key']; // @codingStandardsIgnoreLine

		// create credentials using values supplied in setAuthConfig .
		if ( $signingKey ) { // @codingStandardsIgnoreLine
			$serviceAccountCredentials = array( // @codingStandardsIgnoreLine
				'client_id'    => $this->config['client_id'],
				'client_email' => $this->config['client_email'],
				'private_key'  => $signingKey, // @codingStandardsIgnoreLine
				'type'         => 'service_account',
			);
			$credentials               = CredentialsLoader::makeCredentials( $scopes, $serviceAccountCredentials ); // @codingStandardsIgnoreLine
		} else {
	...
	/**
	 * This function is to get auth handler .
	 */
	protected function getAuthHandler() { // @codingStandardsIgnoreLine
	...
	/**
	 * This function is to create user refresh credentials .
	 *
	 * @param string $scope .
	 * @param string $refreshToken .
	 */
	private function createUserRefreshCredentials( $scope, $refreshToken ) { // @codingStandardsIgnoreLine
		$creds = array_filter(
			array(
