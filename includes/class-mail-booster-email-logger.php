 ...
 */
class Mail_Booster_Email_Logger {

	/**
	 * Load the logger.
	 */
	public function load_emails_mail_booster() {
	...
	public function log_email_failed( $wp_error ) {
		...
		if ( 'enable' === $settings_array_unserialized['monitor_email_logs'] ) {
			if ( isset( $mb_insert_id ) ) {
				...
				$wpdb->update( mail_booster_logs(), $email_logs_data_array, $where );// db call ok; no-cache ok.
			}
		}
	}
	/**
	...
	public function log_email( $mail_info ) {
		...
		$data = array(
			'attachments' => ( '' !== $mail_info['attachments'] ) ? 'true' : 'false',
